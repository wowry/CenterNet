...
class SpectralNormConvLoadStateDictPreHook(object):
    ...
    def __call__(
        ...
        if version is None or version < 1:
            with torch.no_grad():
                key_weight = prefix + fn.name
                key_weight_orig = key_weight + "_orig"
                key_u = key_weight + "_u"
                if key_weight not in state_dict \
                or key_weight_orig not in state_dict \
                or key_u not in state_dict:
                    return
                weight_orig = state_dict[key_weight_orig]
                weight = state_dict.pop(key_weight)
